...
@Component({
  selector: 'app-delayed-cassette-list',
  standalone: true,
  imports: [
    CommonModule,
    MatTableModule,
  ...
  templateUrl: './delayed-cassette-list.component.html',
  styleUrl: './delayed-cassette-list.component.css'
})
export class DelayedCassetteListComponent {
  borrows: BorrowDTO[] = [];
  displayedColumns: string[] = ['id', 'borrowDate', 'returnDate', 'member', 'cassette', 'delay'];
  ...
  loadBorrows(): void {
    ...
    this.borrowService.getAll().subscribe({
        next: (borrows) => {
          this.borrows = borrows.filter(borrow => borrow.cassette != null);
